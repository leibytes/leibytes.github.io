...
本文面向后端、运维、前端开发人员，基于nginx 1\.18\+稳定版本，聚焦工作中**最高频、最易错**的两大核心能力：**本地资源定位**与**反向代理转发**。同时融合全网最全的**正则****l****ocation避坑清单**、**l****ocation五级完整匹配流程**、**proxy\_pass末尾斜杠核心规则**，覆盖静态资源部署、内网外网穿透、前后端分离代理、spa单页应用等主流场景，看完即可解决99%的nginx配置疑难问题。
...
资源定位的核心作用：将客户端请求的URI，精准映射到服务器本地磁盘文件路径，支撑静态资源访问、spa页面部署等场景，核心三要素：**l****ocation匹配规则、Root/Alias路径映射、****t****ry\_files兜底机制**。
...
2. **第二步：前缀匹配****l****ocation**（包括 `^~`阻断正则前缀、普通前缀 `/xxx`），筛选出**最长匹配前缀**

    - 若最长前缀是 `^~`：直接终止匹配，**跳过所有正则**，流程结束

    - 若最长前缀是普通前缀（非 `/` 兜底）：临时保存为候选，**进入正则匹配阶段**

3. **第三步：正则匹配****l****ocation \~ / \~\***，**从上到下顺序匹配**，命中第一个正则立即生效，覆盖普通前缀候选，流程结束
...
- **请求 /gwzx\-front/login**：最长前缀为 `^~ /gwzx-front/`，阻断正则，前缀生效
...
文件绝对路径 = **r****oot配置路径 \+ 完整请求URI**，适用于URI与本地目录结构一致的场景。
...
文件绝对路径 = **a****lias配置路径 \+ URI剔除****l****ocation前缀后的剩余路径**，适用于URI与本地目录不一致的映射场景。
...
业务匹配路径：`/gwzx-front/`，请求uri：`/gwzx-front/login`，后端服务：`1``72.18``.0.``7``:9500`（**后端原生路由自带 /gwzx\-front/ 前缀**）
...
**最终转发地址**：`http://1``72.18``.0.``7``:9500/gwzx-front/login`
